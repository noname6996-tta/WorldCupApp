...
fun MediaMetadataCompat.toMediaSource(dataSourceFactory: DataSource.Factory) =
    if (!mediaUri.toString().endsWith(".m3u8")) {
        ProgressiveMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.Builder().setUri(mediaUri).build())
    } else {
        HlsMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.Builder().setUri(mediaUri).build())
    }
...
//
//fun MediaMetadataCompat.Builder.from(exoPlayer: ExoPlayer): MediaMetadataCompat.Builder {
//    duration = exoPlayer.duration
//    trackNumber = exoPlayer.currentPosition
//    return this
//}
